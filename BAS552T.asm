$mod51
;                                                             Ver.0.3 2023.05.07
; WCH CH552T/G 対応バージョン(SFRアクセス機能付)
;   Internal Clock 24MHz, CPU Clock(Fsys) 24MHz(Default->Extend)
...
CLOCK_CFG	EQU	0B9H	; System clock configuration register
...
;
;=== CH552 Added =====
ERAMEND	EQU	03FFH ; EX-RAM last addr (1KB)
;=====================
;
	;**************************************************************
	;
...
;
;=== CH552 Added =====
	acall	setfsys	; Set CPU Clock 24MHz(Max Speed)
;=====================
	;
...
;
	CJNE	R3,#HIGH ERAMEND,CR0	; Stop the test at 03FFH because
	CJNE	R1,#LOW ERAMEND-1,CR0	; EXT-RAM end
;========================
;
;*****************************************************************************
;
...
;-- insert for AT98LP52 --------------------------------------------------
;
;BG1:	mov	r6,#0		; Speed Table Index
;	mov	r3,#0FFH	; Set R3 = RCAP2H
;
;BG11:	mov	dptr,#SPEED_TBL	; Speed Table
;	mov	a,r6		; Speed Table Index
;	movc	a,@a+dptr	; Fetch Speed Table
;	inc	r6
;	jz	bg3		; Table End, Use Last Speed
;
;	mov	r1,a		; Set R1 = RCAP2L
;	mov	t2con,#34h	;configure Timer2 as baudrate generator
;	call	RCL		;LOAD THE TIMER
;
;	mov	dptr,#TEST_MSG	; Put Test Message
;	mov	r7,#0		; Message Text Index
;
;BG13:	mov	a,r7		; Text Index
;	movc	a,@a+dptr	; Fetch Message String
;	jz	BG14		; String End ?
;	jnb	TI,$		; Wait Until Transmmit End
;	clr	TI		; Clear TX Flag
;	mov	SBUF,a	 	; Output Character
;	inc	r7		; Next Character
;	sjmp	BG13
;
;BG14:	jnb	RI,$		; Wait Until Receive End
;	mov	a,SBUF		; Get Receive Character
;	clr	RI		; Reset Flag
;	anl	a,#07FH		; Clear Bit-7
;	cjne	a,#'y',BG11	; Ok?, Test Next Speed
;	sjmp	BG3
;
;SPEED_TBL:	db	0A0H	; RCAP2H,L = FFA0H (19.2Kbps, X'TAL 29.4912MHz, Fast-Mode)
;		db	0B8H	; RCAP2H,L = FFB8H (19.2Kbps, X'TAL 22.1184MHz, Fast-Mode)
;		db	0D0H	; RCAP2H,L = FFD0H (19.2Kbbs, X'TAL 29.4912MHz, 8051-Mode)
;		db	0DCH	; RCAP2H,L = FFDCH (19.2Kbps, X'TAL 22.1184MHz, 8051-Mode or 11.0592MHz, Fast-Mode)
;		db	00h	; Table End

;-- CH552 Insert Start ---------------------------------------------------------
;===============================================================================
;  CH552 Set Fixed baudrate 19.23Kbps
;  RCAP2H,L = 65536 - (Fsys/16/Baud Rate) : When Fsys = 24MHz, bTMR_CLK=1,bT2_CLK=1
;===============================================================================
baud19K	equ	0ffb2h		; RCAP2H,L Value of 19.23Kbps (FFB2H = 65458)
;
BG1:
	orl		t2mod,#0C0H		; Set bTMR_CLK, bT2_CLK ( Set Timer2 Clock, MAX Speed = Fsys = 24MHz )
	mov		r3,#high baud19k	; Set R3 = RCAP2H = FFH
	mov		r1,#low baud19k		; Set R1 = RCAP2L = B2H
	mov		t2con,#34h		; configure Timer2 as baudrate generator
	call	RCL				; LOAD THE TIMER
...
;===============================================================================
;  CH552 System Clock (Fsys) Up 6MHz -> 24MHz
;===============================================================================
setfsys:
	mov	a,CLOCK_CFG		; Get CLOCK_CFG
	anl	a,#0F8H			;  Clear MASK_SYS_CK_SEL
	orl	a,#006H			;  Set MASK_SYS_CK_SEL = 110B(24MHz)

	mov	SAFE_MOD,#055H	; Enter Safe_Mode Key-1
	mov	SAFE_MOD,#0AAH	; Enter Safe_Mode Key-2
	...
	org	0467h		; Insert Dummy Space, 0467H = Original BG3 Address
;
;-- insert end --------------------------------------------------------------
;
...
TM_TOP: DS	1
;
; ---- BASIC-52 Mainroutine End ----

	CSEG	; Directive is used to select the Program Memory Space.
...
;temp1		equ	11			;Temp. storage. ; *** alredy definition

;xbit		bit	45			;Command extension present. ; *** already definition
...
;error:		setb	34H			;Read string from code memory. ; *** duplicate name
error9:		setb	34H			;Read string from code memory.
